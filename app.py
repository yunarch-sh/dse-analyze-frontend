    ...
    with st.form("login"):
        ...
        if submitted:
            if (username == st.secrets["LOGIN"]["LOGIN_USER"] 
                ...
                # toggle refresh key to force rerun
...
if st.sidebar.button("Log Out"):
    ...
    st.experimental_rerun()  # dummy safe
...
# Initialize refresh key and last refresh timestamp
...
# ---------------- DETAILED VIEW ----------------
stock_list = sorted(analysis_df["Stock"].unique()) if not analysis_df.empty else ["No Data"]
...
if not df_sub.empty:
    ...
    fig_hist.add_trace(go.Bar(x=df_sub["captured_at"], y=df_sub["VOLUME"], name="Volume", yaxis="y2", opacity=0.3, marker_color="#636EFA"))
