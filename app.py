...
selected_stock = st.selectbox("🔍 Select Stock for Detailed Analysis", stock_list)
...
if selected_stock != "No Data":
    ...
    fig_p.update_layout(
        ...
        barmode="group", 
        height=350 + (len(profile_data) * 30),
        xaxis=dict(title="Volume", titlefont=dict(color="#636EFA")),
        xaxis2=dict(title="Stay (Mins)", titlefont=dict(color="#EF553B"), overlaying="x", side="top", showgrid=False),
        yaxis=dict(title="Price", type="category"),
...
st.divider()
st.caption(f"Range: {display_start} to {display_end} | Current Dhaka Time: {now_dhaka.strftime('%H:%M:%S')}")
