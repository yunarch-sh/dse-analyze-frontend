...
# ---------------- EXCEL VOL DIFF HELPER ----------------
def excel_vol_diff(grp):
    """
    Forward difference: volume at current row = next row cumulative - current row cumulative.
    For the last row of each group, use backward diff to avoid losing the final tick's volume.
    This ensures sum(VOL_DIFF_EXCEL) == last cumulative VOLUME for every stock.
    """
    fwd = grp.shift(-1) - grp  # next - current (forward look)
    if len(grp) > 1:
        # Last row: assign same delta as the previous backward diff
        fwd.iloc[-1] = grp.iloc[-1] - grp.iloc[-2]
    else:
        fwd.iloc[-1] = 0
    return fwd
...
if not full_day_df.empty:
    # --- METHOD 1: PDB Logic (Backward Diff) ---
    # Volume traded at current row = current cumulative - previous cumulative
    ...
    # --- METHOD 2: Excel Logic (Forward Diff — FIXED) ---
    # Volume at current row = next row cumulative - current row cumulative
    # Last row uses backward diff instead of being zeroed out, preserving total volume integrity
    full_day_df["VOL_DIFF_EXCEL"] = (
        full_day_df.groupby("TRADING CODE")["VOLUME"]
        .transform(excel_vol_diff)
        .fillna(0)
        .clip(lower=0)  # Remove negative values from data feed anomalies
    )
...
# ---------------- SANITY CHECK (optional — remove after verifying) ----------------
# Uncomment below to debug volume mismatch per stock:
# if not full_day_df.empty:
#     check = full_day_df.groupby("TRADING CODE").agg(
#         cumulative_vol=("VOLUME", "last"),
#         sum_pdb_diff=("VOL_DIFF_PDB", "sum"),
#         sum_excel_diff=("VOL_DIFF_EXCEL", "sum")
#     )
...
if "PDB ALL Price" in display_options:
    if selected_stock != "No Data" and not df_sub.empty:
        ...
        full_profile["Vol % of Total"] = (
            (full_profile["Vol_Traded"] / total_volume_pdb * 100) if total_volume_pdb > 0 else 0
        )
...
# ---------------- EXCEL APPROACH PROFILE (FIXED Forward Diff) ----------------
if "Excel Approach Profile" in display_options:
    if selected_stock != "No Data" and not df_sub.empty:
        st.subheader(f"📊 EXCEL APPROACH (Forward Difference — Fixed) — {selected_stock}")

        st.info("""
        **Excel Logic (Fixed):** Replicates =(Next Row Volume − Current Row Volume) for each row.
        The last row now uses a backward diff instead of being zeroed out, ensuring
        **sum of individual price volumes = total cumulative volume** for every stock.
        """)
        ...
        excel_profile["Vol % of Total"] = (
            (excel_profile["Vol_Traded"] / total_volume_excel * 100) if total_volume_excel > 0 else 0
        )
