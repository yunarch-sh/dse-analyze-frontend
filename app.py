...
def pdb_vol_diff(grp):
    ...
    First row of each stock: use its own cumulative volume as the delta
    (captures all volume traded before the first captured tick in the window).
    Subsequent rows: current cumulative - previous cumulative.
    Negative values (data feed resets) are clipped to 0.
    """
    diff = grp.diff()
    # For the very first row there is no previous row → use cumulative volume itself
    diff.iloc[0] = grp.iloc[0]
    return diff.clip(lower=0)


def excel_vol_diff(grp):
    """
    Forward diff within the filtered window.
    Volume at row N = cumulative[N+1] - cumulative[N].
    The last row has no next row, so its delta is set to 0 (unknown, not fabricated).
    This avoids the double-counting bug of using a backward fallback on the last row.
    Negative values (data feed resets) are clipped to 0.
    """
    fwd = grp.shift(-1) - grp
    fwd.iloc[-1] = 0          # last row: no next tick → volume unknown, do NOT backward-fill
    return fwd.clip(lower=0)
...
# KEY FIX: diffs are computed on the FILTERED slice, not the full day.
# This ensures:
#   • First row delta = its own cumulative volume (not vs a row outside the window)
#   • Last row delta  = 0 (forward diff, not double-counted backward fallback)
#   • sum(VOL_DIFF) ≈ last cumulative VOLUME for every stock inside the window
...
if not full_day_df.empty:
    ...
    if not raw_df.empty:
        ...
        # Excel (Forward Diff) — computed on filtered window, last row = 0
...
# Uncomment to verify sum(VOL_DIFF) == last cumulative VOLUME per stock:
# if not raw_df.empty:
#     check = raw_df.groupby("TRADING CODE").agg(
#         last_cumulative=("VOLUME", "last"),
#         sum_pdb=("VOL_DIFF_PDB", "sum"),
#         sum_excel=("VOL_DIFF_EXCEL", "sum"),
#     )
#     check["pdb_ok"]   = check["sum_pdb"]   == check["last_cumulative"]
#     check["excel_ok"] = check["sum_excel"]  == check["last_cumulative"]
#     st.write("🔍 Volume Sanity Check", check)
...
if "Excel Approach Profile" in display_options:
    if selected_stock != "No Data" and not df_sub.empty:
        st.subheader(f"📊 Excel Approach (Forward Diff — Fixed) — {selected_stock}")

        st.info(
            "**Excel Logic (Fixed):** Forward diff = Next cumulative − Current cumulative. "
            "Last row is set to **0** (not backward-filled) to avoid double-counting the final interval."
        )
